...
@fieldwise_init
struct Finger(ImplicitlyCopyable, Movable):
...
fn get_touch_devices(count: Ptr[c_int, mut=True], out ret: Ptr[TouchID, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (count: Ptr[c_int, mut=True]) -> Ptr[TouchID, mut=True]]("SDL_GetTouchDevices")(count)
    if not ret:
...
fn get_touch_device_name(touch_id: TouchID, out ret: Ptr[c_char, mut=False]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (touch_id: TouchID) -> Ptr[c_char, mut=False]]("SDL_GetTouchDeviceName")(touch_id)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn get_touch_device_type(touch_id: TouchID) -> TouchDeviceType:
    ...
    return _get_sdl_handle()[].get_function[fn (touch_id: TouchID) -> TouchDeviceType]("SDL_GetTouchDeviceType")(touch_id)
...
fn get_touch_fingers(touch_id: TouchID, count: Ptr[c_int, mut=True], out ret: Ptr[Ptr[Finger, mut=True], mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (touch_id: TouchID, count: Ptr[c_int, mut=True]) -> Ptr[Ptr[Finger, mut=True], mut=True]]("SDL_GetTouchFingers")(touch_id, count)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
