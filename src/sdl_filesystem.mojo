...
fn get_base_path() -> Ptr[c_char, mut=False]:
    ...
    **Nintendo 3DS Specific Functionality**: This function returns "romfs"
    ...
    return _get_sdl_handle()[].get_function[fn () -> Ptr[c_char, mut=False]]("SDL_GetBasePath")()
...
fn get_pref_path(var org: String, var app: String) -> Ptr[c_char, mut=True]:
    ...
    return _get_sdl_handle()[].get_function[fn (org: Ptr[c_char, mut=False], app: Ptr[c_char, mut=False]) -> Ptr[c_char, mut=True]]("SDL_GetPrefPath")(org.unsafe_cstr_ptr(), app.unsafe_cstr_ptr())
...
fn get_user_folder(folder: Folder) -> Ptr[c_char, mut=False]:
    ...
    return _get_sdl_handle()[].get_function[fn (folder: Folder) -> Ptr[c_char, mut=False]]("SDL_GetUserFolder")(folder)
...
@fieldwise_init
struct PathInfo(ImplicitlyCopyable, Movable):
...
fn create_directory(var path: String) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (path: Ptr[c_char, mut=False]) -> Bool]("SDL_CreateDirectory")(path.unsafe_cstr_ptr())
    if not ret:
...
fn enumerate_directory(var path: String, callback: EnumerateDirectoryCallback, userdata: Ptr[NoneType, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (path: Ptr[c_char, mut=False], callback: EnumerateDirectoryCallback, userdata: Ptr[NoneType, mut=True]) -> Bool]("SDL_EnumerateDirectory")(path.unsafe_cstr_ptr(), callback, userdata)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn remove_path(var path: String) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (path: Ptr[c_char, mut=False]) -> Bool]("SDL_RemovePath")(path.unsafe_cstr_ptr())
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn rename_path(var oldpath: String, var newpath: String) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (oldpath: Ptr[c_char, mut=False], newpath: Ptr[c_char, mut=False]) -> Bool]("SDL_RenamePath")(oldpath.unsafe_cstr_ptr(), newpath.unsafe_cstr_ptr())
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn copy_file(var oldpath: String, var newpath: String) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (oldpath: Ptr[c_char, mut=False], newpath: Ptr[c_char, mut=False]) -> Bool]("SDL_CopyFile")(oldpath.unsafe_cstr_ptr(), newpath.unsafe_cstr_ptr())
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn get_path_info(var path: String, info: Ptr[PathInfo, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (path: Ptr[c_char, mut=False], info: Ptr[PathInfo, mut=True]) -> Bool]("SDL_GetPathInfo")(path.unsafe_cstr_ptr(), info)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn glob_directory(var path: String, var pattern: String, flags: GlobFlags, count: Ptr[c_int, mut=True], out ret: Ptr[Ptr[c_char, mut=True], mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (path: Ptr[c_char, mut=False], pattern: Ptr[c_char, mut=False], flags: GlobFlags, count: Ptr[c_int, mut=True]) -> Ptr[Ptr[c_char, mut=True], mut=True]]("SDL_GlobDirectory")(path.unsafe_cstr_ptr(), pattern.unsafe_cstr_ptr(), flags, count)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn get_current_directory() -> Ptr[c_char, mut=True]:
    ...
    return _get_sdl_handle()[].get_function[fn () -> Ptr[c_char, mut=True]]("SDL_GetCurrentDirectory")()
