...
@fieldwise_init
struct Renderer(ImplicitlyCopyable, Movable):
...
@fieldwise_init
struct Texture(ImplicitlyCopyable, Movable):
...
fn get_num_render_drivers() -> c_int:
    ...
    return _get_sdl_handle()[].get_function[fn () -> c_int]("SDL_GetNumRenderDrivers")()
...
fn get_render_driver(index: c_int) -> Ptr[c_char, mut=False]:
    ...
    return _get_sdl_handle()[].get_function[fn (index: c_int) -> Ptr[c_char, mut=False]]("SDL_GetRenderDriver")(index)
...
fn create_window_and_renderer(var title: String, width: c_int, height: c_int, window_flags: WindowFlags, window: Ptr[Ptr[Window, mut=True], mut=True], renderer: Ptr[Ptr[Renderer, mut=True], mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (title: Ptr[c_char, mut=False], width: c_int, height: c_int, window_flags: WindowFlags, window: Ptr[Ptr[Window, mut=True], mut=True], renderer: Ptr[Ptr[Renderer, mut=True], mut=True]) -> Bool]("SDL_CreateWindowAndRenderer")(title.unsafe_cstr_ptr(), width, height, window_flags, window, renderer)
    if not ret:
...
fn create_renderer(window: Ptr[Window, mut=True], var name: String) -> Ptr[Renderer, mut=True]:
    ...
    return _get_sdl_handle()[].get_function[fn (window: Ptr[Window, mut=True], name: Ptr[c_char, mut=False]) -> Ptr[Renderer, mut=True]]("SDL_CreateRenderer")(window, name.unsafe_cstr_ptr())
...
fn create_renderer_with_properties(props: PropertiesID) -> Ptr[Renderer, mut=True]:
    ...
    return _get_sdl_handle()[].get_function[fn (props: PropertiesID) -> Ptr[Renderer, mut=True]]("SDL_CreateRendererWithProperties")(props)
...
fn create_software_renderer(surface: Ptr[Surface, mut=True]) -> Ptr[Renderer, mut=True]:
    ...
    return _get_sdl_handle()[].get_function[fn (surface: Ptr[Surface, mut=True]) -> Ptr[Renderer, mut=True]]("SDL_CreateSoftwareRenderer")(surface)
...
fn get_renderer(window: Ptr[Window, mut=True], out ret: Ptr[Renderer, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (window: Ptr[Window, mut=True]) -> Ptr[Renderer, mut=True]]("SDL_GetRenderer")(window)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn get_render_window(renderer: Ptr[Renderer, mut=True], out ret: Ptr[Window, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True]) -> Ptr[Window, mut=True]]("SDL_GetRenderWindow")(renderer)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn get_renderer_name(renderer: Ptr[Renderer, mut=True], out ret: Ptr[c_char, mut=False]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True]) -> Ptr[c_char, mut=False]]("SDL_GetRendererName")(renderer)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn get_renderer_properties(renderer: Ptr[Renderer, mut=True]) -> PropertiesID:
    ...
    return _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True]) -> PropertiesID]("SDL_GetRendererProperties")(renderer)
...
fn get_render_output_size(renderer: Ptr[Renderer, mut=True], w: Ptr[c_int, mut=True], h: Ptr[c_int, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], w: Ptr[c_int, mut=True], h: Ptr[c_int, mut=True]) -> Bool]("SDL_GetRenderOutputSize")(renderer, w, h)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn get_current_render_output_size(renderer: Ptr[Renderer, mut=True], w: Ptr[c_int, mut=True], h: Ptr[c_int, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], w: Ptr[c_int, mut=True], h: Ptr[c_int, mut=True]) -> Bool]("SDL_GetCurrentRenderOutputSize")(renderer, w, h)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn create_texture(renderer: Ptr[Renderer, mut=True], format: PixelFormat, access: TextureAccess, w: c_int, h: c_int, out ret: Ptr[Texture, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], format: PixelFormat, access: TextureAccess, w: c_int, h: c_int) -> Ptr[Texture, mut=True]]("SDL_CreateTexture")(renderer, format, access, w, h)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn create_texture_from_surface(renderer: Ptr[Renderer, mut=True], surface: Ptr[Surface, mut=True], out ret: Ptr[Texture, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], surface: Ptr[Surface, mut=True]) -> Ptr[Texture, mut=True]]("SDL_CreateTextureFromSurface")(renderer, surface)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn create_texture_with_properties(renderer: Ptr[Renderer, mut=True], props: PropertiesID, out ret: Ptr[Texture, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], props: PropertiesID) -> Ptr[Texture, mut=True]]("SDL_CreateTextureWithProperties")(renderer, props)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn get_texture_properties(texture: Ptr[Texture, mut=True]) -> PropertiesID:
    ...
    return _get_sdl_handle()[].get_function[fn (texture: Ptr[Texture, mut=True]) -> PropertiesID]("SDL_GetTextureProperties")(texture)
...
fn get_renderer_from_texture(texture: Ptr[Texture, mut=True]) -> Ptr[Renderer, mut=True]:
    ...
    return _get_sdl_handle()[].get_function[fn (texture: Ptr[Texture, mut=True]) -> Ptr[Renderer, mut=True]]("SDL_GetRendererFromTexture")(texture)
...
fn get_texture_size(texture: Ptr[Texture, mut=True], w: Ptr[c_float, mut=True], h: Ptr[c_float, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (texture: Ptr[Texture, mut=True], w: Ptr[c_float, mut=True], h: Ptr[c_float, mut=True]) -> Bool]("SDL_GetTextureSize")(texture, w, h)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn set_texture_color_mod(texture: Ptr[Texture, mut=True], r: UInt8, g: UInt8, b: UInt8) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (texture: Ptr[Texture, mut=True], r: UInt8, g: UInt8, b: UInt8) -> Bool]("SDL_SetTextureColorMod")(texture, r, g, b)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn set_texture_color_mod_float(texture: Ptr[Texture, mut=True], r: c_float, g: c_float, b: c_float) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (texture: Ptr[Texture, mut=True], r: c_float, g: c_float, b: c_float) -> Bool]("SDL_SetTextureColorModFloat")(texture, r, g, b)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn get_texture_color_mod(texture: Ptr[Texture, mut=True], r: Ptr[UInt8, mut=True], g: Ptr[UInt8, mut=True], b: Ptr[UInt8, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (texture: Ptr[Texture, mut=True], r: Ptr[UInt8, mut=True], g: Ptr[UInt8, mut=True], b: Ptr[UInt8, mut=True]) -> Bool]("SDL_GetTextureColorMod")(texture, r, g, b)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn get_texture_color_mod_float(texture: Ptr[Texture, mut=True], r: Ptr[c_float, mut=True], g: Ptr[c_float, mut=True], b: Ptr[c_float, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (texture: Ptr[Texture, mut=True], r: Ptr[c_float, mut=True], g: Ptr[c_float, mut=True], b: Ptr[c_float, mut=True]) -> Bool]("SDL_GetTextureColorModFloat")(texture, r, g, b)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn set_texture_alpha_mod(texture: Ptr[Texture, mut=True], alpha: UInt8) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (texture: Ptr[Texture, mut=True], alpha: UInt8) -> Bool]("SDL_SetTextureAlphaMod")(texture, alpha)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn set_texture_alpha_mod_float(texture: Ptr[Texture, mut=True], alpha: c_float) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (texture: Ptr[Texture, mut=True], alpha: c_float) -> Bool]("SDL_SetTextureAlphaModFloat")(texture, alpha)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn get_texture_alpha_mod(texture: Ptr[Texture, mut=True], alpha: Ptr[UInt8, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (texture: Ptr[Texture, mut=True], alpha: Ptr[UInt8, mut=True]) -> Bool]("SDL_GetTextureAlphaMod")(texture, alpha)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn get_texture_alpha_mod_float(texture: Ptr[Texture, mut=True], alpha: Ptr[c_float, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (texture: Ptr[Texture, mut=True], alpha: Ptr[c_float, mut=True]) -> Bool]("SDL_GetTextureAlphaModFloat")(texture, alpha)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn set_texture_blend_mode(texture: Ptr[Texture, mut=True], blend_mode: BlendMode) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (texture: Ptr[Texture, mut=True], blend_mode: BlendMode) -> Bool]("SDL_SetTextureBlendMode")(texture, blend_mode)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn get_texture_blend_mode(texture: Ptr[Texture, mut=True], blend_mode: Ptr[BlendMode, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (texture: Ptr[Texture, mut=True], blend_mode: Ptr[BlendMode, mut=True]) -> Bool]("SDL_GetTextureBlendMode")(texture, blend_mode)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn set_texture_scale_mode(texture: Ptr[Texture, mut=True], scale_mode: ScaleMode) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (texture: Ptr[Texture, mut=True], scale_mode: ScaleMode) -> Bool]("SDL_SetTextureScaleMode")(texture, scale_mode)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn get_texture_scale_mode(texture: Ptr[Texture, mut=True], scale_mode: Ptr[ScaleMode, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (texture: Ptr[Texture, mut=True], scale_mode: Ptr[ScaleMode, mut=True]) -> Bool]("SDL_GetTextureScaleMode")(texture, scale_mode)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn update_texture(texture: Ptr[Texture, mut=True], rect: Ptr[Rect, mut=False], pixels: Ptr[NoneType, mut=False], pitch: c_int) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (texture: Ptr[Texture, mut=True], rect: Ptr[Rect, mut=False], pixels: Ptr[NoneType, mut=False], pitch: c_int) -> Bool]("SDL_UpdateTexture")(texture, rect, pixels, pitch)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn update_yuv_texture(texture: Ptr[Texture, mut=True], rect: Ptr[Rect, mut=False], y_plane: Ptr[UInt8, mut=False], y_pitch: c_int, uplane: Ptr[UInt8, mut=False], upitch: c_int, vplane: Ptr[UInt8, mut=False], vpitch: c_int) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (texture: Ptr[Texture, mut=True], rect: Ptr[Rect, mut=False], y_plane: Ptr[UInt8, mut=False], y_pitch: c_int, uplane: Ptr[UInt8, mut=False], upitch: c_int, vplane: Ptr[UInt8, mut=False], vpitch: c_int) -> Bool]("SDL_UpdateYUVTexture")(texture, rect, y_plane, y_pitch, uplane, upitch, vplane, vpitch)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn update_nv_texture(texture: Ptr[Texture, mut=True], rect: Ptr[Rect, mut=False], y_plane: Ptr[UInt8, mut=False], y_pitch: c_int, uv_plane: Ptr[UInt8, mut=False], uv_pitch: c_int) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (texture: Ptr[Texture, mut=True], rect: Ptr[Rect, mut=False], y_plane: Ptr[UInt8, mut=False], y_pitch: c_int, uv_plane: Ptr[UInt8, mut=False], uv_pitch: c_int) -> Bool]("SDL_UpdateNVTexture")(texture, rect, y_plane, y_pitch, uv_plane, uv_pitch)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn lock_texture(texture: Ptr[Texture, mut=True], rect: Ptr[Rect, mut=False], pixels: Ptr[Ptr[NoneType, mut=True], mut=True], pitch: Ptr[c_int, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (texture: Ptr[Texture, mut=True], rect: Ptr[Rect, mut=False], pixels: Ptr[Ptr[NoneType, mut=True], mut=True], pitch: Ptr[c_int, mut=True]) -> Bool]("SDL_LockTexture")(texture, rect, pixels, pitch)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn lock_texture_to_surface(texture: Ptr[Texture, mut=True], rect: Ptr[Rect, mut=False], surface: Ptr[Ptr[Surface, mut=True], mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (texture: Ptr[Texture, mut=True], rect: Ptr[Rect, mut=False], surface: Ptr[Ptr[Surface, mut=True], mut=True]) -> Bool]("SDL_LockTextureToSurface")(texture, rect, surface)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn unlock_texture(texture: Ptr[Texture, mut=True]) -> None:
    ...
    **Warning**: Please note that SDL_LockTexture() is intended to be
    ...
    return _get_sdl_handle()[].get_function[fn (texture: Ptr[Texture, mut=True]) -> None]("SDL_UnlockTexture")(texture)
...
fn set_render_target(renderer: Ptr[Renderer, mut=True], texture: Ptr[Texture, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], texture: Ptr[Texture, mut=True]) -> Bool]("SDL_SetRenderTarget")(renderer, texture)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn get_render_target(renderer: Ptr[Renderer, mut=True]) -> Ptr[Texture, mut=True]:
    ...
    return _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True]) -> Ptr[Texture, mut=True]]("SDL_GetRenderTarget")(renderer)
...
fn set_render_logical_presentation(renderer: Ptr[Renderer, mut=True], w: c_int, h: c_int, mode: RendererLogicalPresentation) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], w: c_int, h: c_int, mode: RendererLogicalPresentation) -> Bool]("SDL_SetRenderLogicalPresentation")(renderer, w, h, mode)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn get_render_logical_presentation(renderer: Ptr[Renderer, mut=True], w: Ptr[c_int, mut=True], h: Ptr[c_int, mut=True], mode: Ptr[RendererLogicalPresentation, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], w: Ptr[c_int, mut=True], h: Ptr[c_int, mut=True], mode: Ptr[RendererLogicalPresentation, mut=True]) -> Bool]("SDL_GetRenderLogicalPresentation")(renderer, w, h, mode)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn get_render_logical_presentation_rect(renderer: Ptr[Renderer, mut=True], rect: Ptr[FRect, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], rect: Ptr[FRect, mut=True]) -> Bool]("SDL_GetRenderLogicalPresentationRect")(renderer, rect)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn render_coordinates_from_window(renderer: Ptr[Renderer, mut=True], window_x: c_float, window_y: c_float, x: Ptr[c_float, mut=True], y: Ptr[c_float, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], window_x: c_float, window_y: c_float, x: Ptr[c_float, mut=True], y: Ptr[c_float, mut=True]) -> Bool]("SDL_RenderCoordinatesFromWindow")(renderer, window_x, window_y, x, y)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn render_coordinates_to_window(renderer: Ptr[Renderer, mut=True], x: c_float, y: c_float, window_x: Ptr[c_float, mut=True], window_y: Ptr[c_float, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], x: c_float, y: c_float, window_x: Ptr[c_float, mut=True], window_y: Ptr[c_float, mut=True]) -> Bool]("SDL_RenderCoordinatesToWindow")(renderer, x, y, window_x, window_y)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn convert_event_to_render_coordinates(renderer: Ptr[Renderer, mut=True], event: Ptr[Event, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], event: Ptr[Event, mut=True]) -> Bool]("SDL_ConvertEventToRenderCoordinates")(renderer, event)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn set_render_viewport(renderer: Ptr[Renderer, mut=True], rect: Ptr[Rect, mut=False]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], rect: Ptr[Rect, mut=False]) -> Bool]("SDL_SetRenderViewport")(renderer, rect)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn get_render_viewport(renderer: Ptr[Renderer, mut=True], rect: Ptr[Rect, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], rect: Ptr[Rect, mut=True]) -> Bool]("SDL_GetRenderViewport")(renderer, rect)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn render_viewport_set(renderer: Ptr[Renderer, mut=True]) -> Bool:
    ...
    return _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True]) -> Bool]("SDL_RenderViewportSet")(renderer)
...
fn get_render_safe_area(renderer: Ptr[Renderer, mut=True], rect: Ptr[Rect, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], rect: Ptr[Rect, mut=True]) -> Bool]("SDL_GetRenderSafeArea")(renderer, rect)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn set_render_clip_rect(renderer: Ptr[Renderer, mut=True], rect: Ptr[Rect, mut=False]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], rect: Ptr[Rect, mut=False]) -> Bool]("SDL_SetRenderClipRect")(renderer, rect)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn get_render_clip_rect(renderer: Ptr[Renderer, mut=True], rect: Ptr[Rect, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], rect: Ptr[Rect, mut=True]) -> Bool]("SDL_GetRenderClipRect")(renderer, rect)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn render_clip_enabled(renderer: Ptr[Renderer, mut=True]) -> Bool:
    ...
    return _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True]) -> Bool]("SDL_RenderClipEnabled")(renderer)
...
fn set_render_scale(renderer: Ptr[Renderer, mut=True], scale_x: c_float, scale_y: c_float) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], scale_x: c_float, scale_y: c_float) -> Bool]("SDL_SetRenderScale")(renderer, scale_x, scale_y)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn get_render_scale(renderer: Ptr[Renderer, mut=True], scale_x: Ptr[c_float, mut=True], scale_y: Ptr[c_float, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], scale_x: Ptr[c_float, mut=True], scale_y: Ptr[c_float, mut=True]) -> Bool]("SDL_GetRenderScale")(renderer, scale_x, scale_y)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn set_render_draw_color(renderer: Ptr[Renderer, mut=True], r: UInt8, g: UInt8, b: UInt8, a: UInt8) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], r: UInt8, g: UInt8, b: UInt8, a: UInt8) -> Bool]("SDL_SetRenderDrawColor")(renderer, r, g, b, a)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn set_render_draw_color_float(renderer: Ptr[Renderer, mut=True], r: c_float, g: c_float, b: c_float, a: c_float) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], r: c_float, g: c_float, b: c_float, a: c_float) -> Bool]("SDL_SetRenderDrawColorFloat")(renderer, r, g, b, a)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn get_render_draw_color(renderer: Ptr[Renderer, mut=True], r: Ptr[UInt8, mut=True], g: Ptr[UInt8, mut=True], b: Ptr[UInt8, mut=True], a: Ptr[UInt8, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], r: Ptr[UInt8, mut=True], g: Ptr[UInt8, mut=True], b: Ptr[UInt8, mut=True], a: Ptr[UInt8, mut=True]) -> Bool]("SDL_GetRenderDrawColor")(renderer, r, g, b, a)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn get_render_draw_color_float(renderer: Ptr[Renderer, mut=True], r: Ptr[c_float, mut=True], g: Ptr[c_float, mut=True], b: Ptr[c_float, mut=True], a: Ptr[c_float, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], r: Ptr[c_float, mut=True], g: Ptr[c_float, mut=True], b: Ptr[c_float, mut=True], a: Ptr[c_float, mut=True]) -> Bool]("SDL_GetRenderDrawColorFloat")(renderer, r, g, b, a)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn set_render_color_scale(renderer: Ptr[Renderer, mut=True], scale: c_float) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], scale: c_float) -> Bool]("SDL_SetRenderColorScale")(renderer, scale)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn get_render_color_scale(renderer: Ptr[Renderer, mut=True], scale: Ptr[c_float, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], scale: Ptr[c_float, mut=True]) -> Bool]("SDL_GetRenderColorScale")(renderer, scale)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn set_render_draw_blend_mode(renderer: Ptr[Renderer, mut=True], blend_mode: BlendMode) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], blend_mode: BlendMode) -> Bool]("SDL_SetRenderDrawBlendMode")(renderer, blend_mode)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn get_render_draw_blend_mode(renderer: Ptr[Renderer, mut=True], blend_mode: Ptr[BlendMode, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], blend_mode: Ptr[BlendMode, mut=True]) -> Bool]("SDL_GetRenderDrawBlendMode")(renderer, blend_mode)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn render_clear(renderer: Ptr[Renderer, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True]) -> Bool]("SDL_RenderClear")(renderer)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn render_point(renderer: Ptr[Renderer, mut=True], x: c_float, y: c_float) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], x: c_float, y: c_float) -> Bool]("SDL_RenderPoint")(renderer, x, y)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn render_points(renderer: Ptr[Renderer, mut=True], points: Ptr[FPoint, mut=False], count: c_int) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], points: Ptr[FPoint, mut=False], count: c_int) -> Bool]("SDL_RenderPoints")(renderer, points, count)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn render_line(renderer: Ptr[Renderer, mut=True], x1: c_float, y1: c_float, x2: c_float, y2: c_float) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], x1: c_float, y1: c_float, x2: c_float, y2: c_float) -> Bool]("SDL_RenderLine")(renderer, x1, y1, x2, y2)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn render_lines(renderer: Ptr[Renderer, mut=True], points: Ptr[FPoint, mut=False], count: c_int) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], points: Ptr[FPoint, mut=False], count: c_int) -> Bool]("SDL_RenderLines")(renderer, points, count)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn render_rect(renderer: Ptr[Renderer, mut=True], rect: Ptr[FRect, mut=False]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], rect: Ptr[FRect, mut=False]) -> Bool]("SDL_RenderRect")(renderer, rect)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn render_rects(renderer: Ptr[Renderer, mut=True], rects: Ptr[FRect, mut=False], count: c_int) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], rects: Ptr[FRect, mut=False], count: c_int) -> Bool]("SDL_RenderRects")(renderer, rects, count)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn render_fill_rect(renderer: Ptr[Renderer, mut=True], rect: Ptr[FRect, mut=False]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], rect: Ptr[FRect, mut=False]) -> Bool]("SDL_RenderFillRect")(renderer, rect)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn render_fill_rects(renderer: Ptr[Renderer, mut=True], rects: Ptr[FRect, mut=False], count: c_int) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], rects: Ptr[FRect, mut=False], count: c_int) -> Bool]("SDL_RenderFillRects")(renderer, rects, count)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn render_texture(renderer: Ptr[Renderer, mut=True], texture: Ptr[Texture, mut=True], srcrect: Ptr[FRect, mut=False], dstrect: Ptr[FRect, mut=False]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], texture: Ptr[Texture, mut=True], srcrect: Ptr[FRect, mut=False], dstrect: Ptr[FRect, mut=False]) -> Bool]("SDL_RenderTexture")(renderer, texture, srcrect, dstrect)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn render_texture_rotated(renderer: Ptr[Renderer, mut=True], texture: Ptr[Texture, mut=True], srcrect: Ptr[FRect, mut=False], dstrect: Ptr[FRect, mut=False], angle: c_double, center: Ptr[FPoint, mut=False], flip: FlipMode) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], texture: Ptr[Texture, mut=True], srcrect: Ptr[FRect, mut=False], dstrect: Ptr[FRect, mut=False], angle: c_double, center: Ptr[FPoint, mut=False], flip: FlipMode) -> Bool]("SDL_RenderTextureRotated")(renderer, texture, srcrect, dstrect, angle, center, flip)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn render_texture_affine(renderer: Ptr[Renderer, mut=True], texture: Ptr[Texture, mut=True], srcrect: Ptr[FRect, mut=False], origin: Ptr[FPoint, mut=False], right: Ptr[FPoint, mut=False], down: Ptr[FPoint, mut=False]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], texture: Ptr[Texture, mut=True], srcrect: Ptr[FRect, mut=False], origin: Ptr[FPoint, mut=False], right: Ptr[FPoint, mut=False], down: Ptr[FPoint, mut=False]) -> Bool]("SDL_RenderTextureAffine")(renderer, texture, srcrect, origin, right, down)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn render_texture_tiled(renderer: Ptr[Renderer, mut=True], texture: Ptr[Texture, mut=True], srcrect: Ptr[FRect, mut=False], scale: c_float, dstrect: Ptr[FRect, mut=False]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], texture: Ptr[Texture, mut=True], srcrect: Ptr[FRect, mut=False], scale: c_float, dstrect: Ptr[FRect, mut=False]) -> Bool]("SDL_RenderTextureTiled")(renderer, texture, srcrect, scale, dstrect)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn render_texture_9grid(renderer: Ptr[Renderer, mut=True], texture: Ptr[Texture, mut=True], srcrect: Ptr[FRect, mut=False], left_width: c_float, right_width: c_float, top_height: c_float, bottom_height: c_float, scale: c_float, dstrect: Ptr[FRect, mut=False]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], texture: Ptr[Texture, mut=True], srcrect: Ptr[FRect, mut=False], left_width: c_float, right_width: c_float, top_height: c_float, bottom_height: c_float, scale: c_float, dstrect: Ptr[FRect, mut=False]) -> Bool]("SDL_RenderTexture9Grid")(renderer, texture, srcrect, left_width, right_width, top_height, bottom_height, scale, dstrect)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn render_geometry(renderer: Ptr[Renderer, mut=True], texture: Ptr[Texture, mut=True], vertices: Ptr[Vertex, mut=False], num_vertices: c_int, indices: Ptr[c_int, mut=False], num_indices: c_int) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], texture: Ptr[Texture, mut=True], vertices: Ptr[Vertex, mut=False], num_vertices: c_int, indices: Ptr[c_int, mut=False], num_indices: c_int) -> Bool]("SDL_RenderGeometry")(renderer, texture, vertices, num_vertices, indices, num_indices)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn render_geometry_raw(renderer: Ptr[Renderer, mut=True], texture: Ptr[Texture, mut=True], xy: Ptr[c_float, mut=False], xy_stride: c_int, color: Ptr[FColor, mut=False], color_stride: c_int, uv: Ptr[c_float, mut=False], uv_stride: c_int, num_vertices: c_int, indices: Ptr[NoneType, mut=False], num_indices: c_int, size_indices: c_int) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], texture: Ptr[Texture, mut=True], xy: Ptr[c_float, mut=False], xy_stride: c_int, color: Ptr[FColor, mut=False], color_stride: c_int, uv: Ptr[c_float, mut=False], uv_stride: c_int, num_vertices: c_int, indices: Ptr[NoneType, mut=False], num_indices: c_int, size_indices: c_int) -> Bool]("SDL_RenderGeometryRaw")(renderer, texture, xy, xy_stride, color, color_stride, uv, uv_stride, num_vertices, indices, num_indices, size_indices)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn render_read_pixels(renderer: Ptr[Renderer, mut=True], rect: Ptr[Rect, mut=False], out ret: Ptr[Surface, mut=True]) raises:
    ...
    **WARNING**: This is a very slow operation, and should not be used
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], rect: Ptr[Rect, mut=False]) -> Ptr[Surface, mut=True]]("SDL_RenderReadPixels")(renderer, rect)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn render_present(renderer: Ptr[Renderer, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True]) -> Bool]("SDL_RenderPresent")(renderer)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn destroy_texture(texture: Ptr[Texture, mut=True]) -> None:
    ...
    return _get_sdl_handle()[].get_function[fn (texture: Ptr[Texture, mut=True]) -> None]("SDL_DestroyTexture")(texture)
...
fn destroy_renderer(renderer: Ptr[Renderer, mut=True]) -> None:
    ...
    return _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True]) -> None]("SDL_DestroyRenderer")(renderer)
...
fn flush_renderer(renderer: Ptr[Renderer, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True]) -> Bool]("SDL_FlushRenderer")(renderer)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn get_render_metal_layer(renderer: Ptr[Renderer, mut=True]) -> Ptr[NoneType, mut=True]:
    ...
    return _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True]) -> Ptr[NoneType, mut=True]]("SDL_GetRenderMetalLayer")(renderer)
...
fn get_render_metal_command_encoder(renderer: Ptr[Renderer, mut=True]) -> Ptr[NoneType, mut=True]:
    ...
    return _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True]) -> Ptr[NoneType, mut=True]]("SDL_GetRenderMetalCommandEncoder")(renderer)
...
fn add_vulkan_render_semaphores(renderer: Ptr[Renderer, mut=True], wait_stage_mask: UInt32, wait_semaphore: Int64, signal_semaphore: Int64) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], wait_stage_mask: UInt32, wait_semaphore: Int64, signal_semaphore: Int64) -> Bool]("SDL_AddVulkanRenderSemaphores")(renderer, wait_stage_mask, wait_semaphore, signal_semaphore)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn set_render_vsync(renderer: Ptr[Renderer, mut=True], vsync: c_int) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], vsync: c_int) -> Bool]("SDL_SetRenderVSync")(renderer, vsync)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn get_render_vsync(renderer: Ptr[Renderer, mut=True], vsync: Ptr[c_int, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], vsync: Ptr[c_int, mut=True]) -> Bool]("SDL_GetRenderVSync")(renderer, vsync)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn render_debug_text(renderer: Ptr[Renderer, mut=True], x: c_float, y: c_float, var str: String) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (renderer: Ptr[Renderer, mut=True], x: c_float, y: c_float, str: Ptr[c_char, mut=False]) -> Bool]("SDL_RenderDebugText")(renderer, x, y, str.unsafe_cstr_ptr())
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
