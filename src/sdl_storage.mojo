...
fn open_title_storage(var override: String, props: PropertiesID, out ret: Ptr[Storage, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (override: Ptr[c_char, mut=False], props: PropertiesID) -> Ptr[Storage, mut=True]]("SDL_OpenTitleStorage")(override.unsafe_cstr_ptr(), props)
    if not ret:
...
fn open_user_storage(var org: String, var app: String, props: PropertiesID, out ret: Ptr[Storage, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (org: Ptr[c_char, mut=False], app: Ptr[c_char, mut=False], props: PropertiesID) -> Ptr[Storage, mut=True]]("SDL_OpenUserStorage")(org.unsafe_cstr_ptr(), app.unsafe_cstr_ptr(), props)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn open_file_storage(var path: String, out ret: Ptr[Storage, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (path: Ptr[c_char, mut=False]) -> Ptr[Storage, mut=True]]("SDL_OpenFileStorage")(path.unsafe_cstr_ptr())
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn open_storage(iface: Ptr[StorageInterface, mut=False], userdata: Ptr[NoneType, mut=True], out ret: Ptr[Storage, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (iface: Ptr[StorageInterface, mut=False], userdata: Ptr[NoneType, mut=True]) -> Ptr[Storage, mut=True]]("SDL_OpenStorage")(iface, userdata)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn close_storage(storage: Ptr[Storage, mut=True]) -> Bool:
    ...
    return _get_sdl_handle()[].get_function[fn (storage: Ptr[Storage, mut=True]) -> Bool]("SDL_CloseStorage")(storage)
...
fn storage_ready(storage: Ptr[Storage, mut=True]) -> Bool:
    ...
    return _get_sdl_handle()[].get_function[fn (storage: Ptr[Storage, mut=True]) -> Bool]("SDL_StorageReady")(storage)
...
fn get_storage_file_size(storage: Ptr[Storage, mut=True], var path: String, length: Ptr[UInt64, mut=True]) -> Bool:
    ...
    return _get_sdl_handle()[].get_function[fn (storage: Ptr[Storage, mut=True], path: Ptr[c_char, mut=False], length: Ptr[UInt64, mut=True]) -> Bool]("SDL_GetStorageFileSize")(storage, path.unsafe_cstr_ptr(), length)
...
fn read_storage_file(storage: Ptr[Storage, mut=True], var path: String, destination: Ptr[NoneType, mut=True], length: UInt64) -> Bool:
    ...
    return _get_sdl_handle()[].get_function[fn (storage: Ptr[Storage, mut=True], path: Ptr[c_char, mut=False], destination: Ptr[NoneType, mut=True], length: UInt64) -> Bool]("SDL_ReadStorageFile")(storage, path.unsafe_cstr_ptr(), destination, length)
...
fn write_storage_file(storage: Ptr[Storage, mut=True], var path: String, source: Ptr[NoneType, mut=False], length: UInt64) -> Bool:
    ...
    return _get_sdl_handle()[].get_function[fn (storage: Ptr[Storage, mut=True], path: Ptr[c_char, mut=False], source: Ptr[NoneType, mut=False], length: UInt64) -> Bool]("SDL_WriteStorageFile")(storage, path.unsafe_cstr_ptr(), source, length)
...
fn create_storage_directory(storage: Ptr[Storage, mut=True], var path: String) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (storage: Ptr[Storage, mut=True], path: Ptr[c_char, mut=False]) -> Bool]("SDL_CreateStorageDirectory")(storage, path.unsafe_cstr_ptr())
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn enumerate_storage_directory(storage: Ptr[Storage, mut=True], var path: String, callback: EnumerateDirectoryCallback, userdata: Ptr[NoneType, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (storage: Ptr[Storage, mut=True], path: Ptr[c_char, mut=False], callback: EnumerateDirectoryCallback, userdata: Ptr[NoneType, mut=True]) -> Bool]("SDL_EnumerateStorageDirectory")(storage, path.unsafe_cstr_ptr(), callback, userdata)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn remove_storage_path(storage: Ptr[Storage, mut=True], var path: String) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (storage: Ptr[Storage, mut=True], path: Ptr[c_char, mut=False]) -> Bool]("SDL_RemoveStoragePath")(storage, path.unsafe_cstr_ptr())
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn rename_storage_path(storage: Ptr[Storage, mut=True], var oldpath: String, var newpath: String) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (storage: Ptr[Storage, mut=True], oldpath: Ptr[c_char, mut=False], newpath: Ptr[c_char, mut=False]) -> Bool]("SDL_RenameStoragePath")(storage, oldpath.unsafe_cstr_ptr(), newpath.unsafe_cstr_ptr())
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn copy_storage_file(storage: Ptr[Storage, mut=True], var oldpath: String, var newpath: String) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (storage: Ptr[Storage, mut=True], oldpath: Ptr[c_char, mut=False], newpath: Ptr[c_char, mut=False]) -> Bool]("SDL_CopyStorageFile")(storage, oldpath.unsafe_cstr_ptr(), newpath.unsafe_cstr_ptr())
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn get_storage_path_info(storage: Ptr[Storage, mut=True], var path: String, info: Ptr[PathInfo, mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (storage: Ptr[Storage, mut=True], path: Ptr[c_char, mut=False], info: Ptr[PathInfo, mut=True]) -> Bool]("SDL_GetStoragePathInfo")(storage, path.unsafe_cstr_ptr(), info)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
fn get_storage_space_remaining(storage: Ptr[Storage, mut=True]) -> UInt64:
    ...
    return _get_sdl_handle()[].get_function[fn (storage: Ptr[Storage, mut=True]) -> UInt64]("SDL_GetStorageSpaceRemaining")(storage)
...
fn glob_storage_directory(storage: Ptr[Storage, mut=True], var path: String, var pattern: String, flags: GlobFlags, count: Ptr[c_int, mut=True], out ret: Ptr[Ptr[c_char, mut=True], mut=True]) raises:
    ...
    ret = _get_sdl_handle()[].get_function[fn (storage: Ptr[Storage, mut=True], path: Ptr[c_char, mut=False], pattern: Ptr[c_char, mut=False], flags: GlobFlags, count: Ptr[c_int, mut=True]) -> Ptr[Ptr[c_char, mut=True], mut=True]]("SDL_GlobStorageDirectory")(storage, path.unsafe_cstr_ptr(), pattern.unsafe_cstr_ptr(), flags, count)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
